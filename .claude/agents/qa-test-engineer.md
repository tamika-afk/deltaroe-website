...
- (2026-09-24) Every checkout/ordering gate gets a PURE classification function with a drill on synthetic items (scripts/test-checkout-stock-gate.ts) — the live path (ION + DB) cannot run in a unit test, so the decision logic is separated from the I/O and tested on its own. Test the made-to-order classes explicitly (saws incl. in-between thickness + coated, WIP, coat-to-order) alongside the one finite-stock case.
