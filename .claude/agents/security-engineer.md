...
## Lessons learned
- (retro 2026-09-21, the 9/18 incident) Email link scanners (Defender Safe Links) EXECUTE one-click GET actions — approval/deny links were auto-clicked 21s after send, recorded as the approver. Every emailed action link must be two-step: GET renders a confirm page, only the POST acts. Scanners follow links, never submit forms. Telltale in audits: action timestamps seconds after email delivery.
- (retro 2026-09-21) Approval-link pattern that works: per-approver HMAC tokens bound to item+action, first-decision-wins recorded WITH who, idempotent re-clicks, and sticky approvals a later stray click can't reverse.
- (retro 2026-09-21, ruled 8/17) Access is opt-in per capability, never implied by a role flag (staff ≠ quote-engine access) — and every new gate reuses the ONE established gate function; ad-hoc checks are how bypasses ship. Every new group/gate grants the owner's account in the same change so features never ship locked away from him.
- (2026-09-21, the Graph mail grant) Least-privilege integration negotiation that wins IT over: drop the scariest scope entirely (no Mail.Send — drafts only), address resources by immutable OBJECT ID not name/email, secrets as runtime-decrypt env vars (never on-disk tokens), a documented kill switch, and a verification suite that PROVES the denied capability fails (the send attempt must be rejected) — proving what SHOULDN'T work is half the audit.
- (deep sweep, Mike 8/24) Accounts are individual per PERSON, never shared per company — revocation must be per-person when someone leaves.
- (2026-09-23, Mike's staff decision tiers) Tiered decision rights are computed on the REAL session identity with Super View OFF, never on the effective identity: canApproveLogins (approver list = the same env list the approval emails go to, TEST_TOOL_APPROVERS) and canDecideForMike (Mike only). The server returns only the queues the caller may act on and the client renders exactly that — a client-side flag is display, never a gate. Same-origin check on every decision POST; a decision never rides on a GET (Defender Safe Links lesson 9/18).

- **Abuse limits must never refuse a real person on a shared IP (9/24 sweep).** Corporate NAT collapses a whole shop onto one address, so per-IP budgets sized for one visitor (5 sign-in links/hour, 6 chat messages/hour, 8/min per form) are reachable in normal use. Size the limit for a building, not a person; key on identity when one exists; and a CAN-SPAM unsubscribe is never rate-limited against the person asking (only a flood bound no human reaches — 60/min — fail OPEN). A limit that would surprise a customer is a business decision to ledger for Mike, not a security default.
