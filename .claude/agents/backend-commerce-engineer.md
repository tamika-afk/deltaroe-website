...
- (2026-09-24, Mike: "someone tried to order a saw as a test and it errors out and says we don't have stock") Syteline carries EVERY slitting saw at 0 on hand — saws are ground to order — so a live on-hand check reads a saw as out of stock. Made-to-order classes (saws via isSawPartNbr, WIP-buildable tools, coat-to-order parts) are exempt from the checkout stock gate BEFORE the live ION read (classifyFulfillment in src/lib/availability.ts; drill `npm run test:stock-gate`). Rule: an ERP on-hand of 0 means "not stocked", never "cannot be ordered" — a manufacturer's storefront must never refuse an order for a made-to-order item.
- (2026-09-24 RULING, supersedes the saw-exemption lesson above — LAW 31) Mike: "all standard tools including end mills, accessories and everything that are not in stock — we allow anyone to order them." There is NO checkout stock gate at all on robbjack.com: every standard part is orderable at any quantity; on-hand only sets the availability label and lead time. Never reintroduce an oversell refusal (guard: npm run test:stock-gate). A manufacturer's storefront turns "not on the shelf" into a lead time, never into a lost order.
- (2026-09-24, Mike: "when did I make the decision to stop an order? I never intentionally made that decision") A behavior that can turn a customer away (refuse an order, block a checkout, hide a product) is a BUSINESS decision, never a builder's default. Never ship one on an assumption, and never record your own verification of it as "the design" — put it to Mike explicitly (needs-mike ledger) and default to ACCEPTING the order until he rules.

- **Nothing is dropped silently (9/24 refusal sweep).** `if (!found || price == null) continue;` in a checkout loop meant a buyer could pay for fewer lines than were in the cart with no message. Any line, file, row or field the server discards must be NAMED back to the user (or reported in the response) — a `continue` on customer input is a bug. Same sweep: five lead forms shared ONE rate-limit bucket keyed on IP (a NAT'd shop that ran two calculators could be refused on the RFQ that mattered) — give every form its own key; and never name a honeypot field `company`/`url`/`website` — browsers autofill those and a real person gets silently discarded as a bot (use a name that matches no autofill heuristic, plus data-lpignore / data-1p-ignore).
- **Every refuse/cap/hide/redirect on a customer path carries its ruling.** The 9/24 sweep found 8 customer-facing limits with no Mike ruling (US/CA-only checkout, 999/50 caps, $50 minimum in Terms vs none in code, portal 30-day quote expiry, 10-piece test-tool cap, oversize shipping unhandled, coated saws >0.750 hidden, NAT-reachable rate limits). When you write one, cite the ruling in the comment or ledger it in data/needs-mike.json as an open question the same day — a limit with no citation is an assumption.
